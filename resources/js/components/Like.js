...
import { geturl , dislike , dolike , isliked } from './products/functions';
// import {geturl , dislike , dolike , isliked} from './functions';

const Like=(props)=>{
    const [like, setlike] = useState(false);
    const [id, setid] = useState(document.getElementById('id').value)
    useEffect(() => {
    ...
    return (
        <span onClick={()=>handlelike()} id={'like'+id} className='unselectable icon-shadow' style={{width:'60px'}}>
